...
class LlamaLlm:
    ...
    def build_parameters(
        ...
        inp = self.tokenizer(
            sents, padding=True, add_special_tokens=True, return_tensors="np", return_length=True
        )
        batch_range = range(len(sents))
        input_ids = inp.input_ids.astype(np.uint32)
        input_len = np.array([[v] for v in inp.length], dtype=np.uint32)
        output_len = np.array([[max_new_tokens] for v in batch_range], dtype=np.uint32)
        temps = np.array([[temperature] for v in batch_range], dtype=np.float32)
        top_ps = np.array([[top_p] for v in batch_range], dtype=np.float32)
        top_ks = np.array([[top_k] for v in batch_range], dtype=np.uint32)
        repetition_penalties = np.array([[repetition_penalty] for v in batch_range], dtype=np.float32)
    ...
    def prepare_tensor(
        ...
        t = client.InferInput(
            param_name, param_value.shape, np_to_triton_dtype(param_value.dtype)
        )
    ...
    def generate(
        ...
        req_params = self.build_parameters(
            sents, max_new_tokens, temperature, top_p, top_k, repetition_penalty
        )
        result_queue = Queue()
        t = threading.Thread(
